...
- AI 평가 (v0.5, BYOK): 내 정보 → "AI 설정"에서 자신의 LLM API 키(Anthropic/OpenAI/호환 엔드포인트)를 넣으면 인박스·파이프라인 상세의 "AI 평가" 버튼으로 분석가와 동일 포맷(등급 5줄)의 평가를 즉시 돌릴 수 있다. 키는 `state.db`에만 저장되고 응답으로 재노출되지 않으며(마스킹), 브라우저 JS는 키를 모른 채 서버 프록시(`/api/ai/chat`, SSE)를 통해 공급자와 대화한다. 대화 API는 v0.6 채팅 UI에서 소비된다.
