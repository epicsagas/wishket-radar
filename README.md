...
> 배포에서는 프로필 편집이 대시보드(설정)나 `wishket-profile` 스킬로만
...
- 설정: 매칭 프로필(구조화 폼 + YAML), 포트폴리오, AI 설정, 프로필 프리셋. 포트폴리오는 공고와 무관한 재사용 자산이라 제안서와 분리한다.
- AI 평가 (v0.5, BYOK): 설정 → "AI 설정"에서 자신의 LLM API 키(Anthropic/OpenAI/호환 엔드포인트)를 넣으면 인박스·파이프라인 상세의 "AI 평가" 버튼으로 분석가와 동일 포맷(등급 5줄)의 평가를 즉시 돌릴 수 있다. 키는 `state.db`에만 저장되고 응답으로 재노출되지 않으며(마스킹), 브라우저 JS는 키를 모른 채 서버 프록시(`/api/ai/chat`, SSE)를 통해 공급자와 대화한다.
- AI 대화 (v0.6): 우하단 플로팅 버튼으로 어느 화면에서든 질문하고, 사이드바 "AI 대화" `#/chats`에서 대화 목록·이어하기·삭제(2단 확인)를 관리한다. 모든 대화에 기술 프로필·공고 캐시 요약·파이프라인 현황이 맥락으로 주입되고("내 프로필에 맞는 공고 top 3" 같은 질문에 근거로 답함), 공고 상세에서 시작하면 본문·조건까지 주입되어 목록에 공고 제목으로 태그된다. 응답은 마크다운으로 렌더된다. 대화·토큰 누적은 SQLite에 영속되며 채팅 헤더에서 볼 수 있다.
- AI 초안·보정·프리셋 (v0.7): 파이프라인 상세·제안서 페이지의 "AI 초안 생성"으로 공고별 제안서 초안을 만든다(기존 편집본은 `.bak` 보존). 설정 화면의 "AI 보정"은 관심·스킵 히스토리를 근거로 키워드 가중치 조정안을 제시하고, 적용은 양식 반영 후 직접 저장. 프로필 프리셋으로 용도별 매칭 설정을 여러 벌 두고 전환하면 스캔·AI 전부 활성 프리셋으로 동작한다.
