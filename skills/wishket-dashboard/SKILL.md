...
description: 위시켓 로컬 대시보드 webui 실행. 지원 파이프라인, 매치 리스트, 제안서, 프로필, 리포트를 브라우저에서 보고 편집한다. "대시보드", "웹 UI 켜줘", "지원 현황 화면으로 보여줘", "dashboard" 등에 사용.
---
...
- 프로필 / 제안서·포트폴리오: 직접 편집 가능 (저장 시 서버가 검증·원자 쓰기, 이전 본문은 `.bak`에 1세대 보관)
